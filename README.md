...
- User tag chips shown below author names in posts (and on profiles)

**Profiles:**
- Forum activity analysis

**User Tagging:**
- Manual tags applied from a user's profile, shown as chips site-wide
- Automatic tags derived from the forums/sub-forums a user posts in most, plus optional per-megathread tags for specific threads you designate
- Editable short names mapping long forum names to compact tag labels
- Passive activity collection while browsing threads, plus opt-in throttled crawl buttons on forums, sub-forums, and individual (mega)threads
- Persistent per-thread dedup so reloads, revisits, and re-crawls never double-count activity
- Hide tag chips globally or per user
- Ships with a prebuilt dataset (activity + tags) so tags work out of the box, generated by a separate offline crawler — nothing is uploaded
- Export/import all tagging data as JSON
